...
const inStockOnly = null; // TODO: 用 R.filter + R.propEq (部分应用)

// 1b: inCategory(category) — 筛选指定分类的商品
function inCategory(category) {
  // TODO: 用 R.filter + R.propEq, 返回柯里化后的函数
}

// 1c: priceAbove(min) — 筛选价格 >= min 的商品
function priceAbove(min) {
  // TODO: 用 R.filter + R.propSatisfies
}

// 练习2: 利用 Ramda 自动柯里化创建数据提取器

// 2a: getProductNames — 提取所有商品的 name 列表
const getProductNames = null; // TODO: 用 R.pluck 或 R.map + R.prop

// 2b: totalPrice — 计算所有商品总价（用 R.reduce + R.prop）
const totalPrice = null; // TODO: 用 R.compose + R.reduce + R.add + R.map + R.prop
...
const divideBy = null; // TODO: 用 R.flip + R.divide

// 练习4: 综合 — 用 Ramda 串联过滤 + 映射 + 统计
// 统计所有在售且价格 >= 100 的商品总价
function totalInStockAbove(minPrice, productList) {
  // TODO: 用 R.pipe 串联 R.filter + R.pluck + R.sum (或 R.reduce)
}
...
describe("练习1: 柯里化过滤器", () => {
  ...
  it("priceAbove 应筛选价格 >= min", () => {
    const result = priceAbove(200)(products);
    assert.equal(result.length, 3);
    result.forEach((p) => assert.ok(p.price >= 200));
  });
});
