...
function repeat(times, fn) {
  // TODO: 执行 fn 共 times 次，将每次结果收集到数组返回
}

// 2b: 实现 twice(fn, x) — 对 x 连续应用 fn 两次，即 fn(fn(x))
function twice(fn, x) {
  // TODO: 对 x 连续应用 fn 两次，即 fn(fn(x))
}
...
function stringifyArray(arr) {
  // TODO: 用 map 将每个数转为字符串
}

// 3c: 求每个数的平方再 +1
function squarePlusOne(arr) {
  // TODO: 用 map 求每个数的平方再 +1
}
