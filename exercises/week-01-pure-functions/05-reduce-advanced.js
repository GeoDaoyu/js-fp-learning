...
function totalElectronics(orderList) {
  // TODO: orderList.filter(...).map(...).reduce(...)
}

// 练习4: 用 reduce 实现 map
function myMap(arr, fn) {
  // TODO: 用 reduce 实现 map 的功能，禁止用 arr.map()
}

// 练习5: 用 reduce 实现 filter
function myFilter(arr, predicate) {
  // TODO: 用 reduce 实现 filter 的功能，禁止用 arr.filter()
}
...
describe('练习4: 用 reduce 实现 map', () => {
  it('功能应与 Array.map 一致', () => {
    ...
    assert.deepEqual(myMap(['a', 'b'], x => x.toUpperCase()), ['A', 'B']);
  });
});
...
describe('练习5: 用 reduce 实现 filter', () => {
  it('功能应与 Array.filter 一致', () => {
    ...
    assert.deepEqual(myFilter(['a', 'bb', 'ccc'], x => x.length > 1), ['bb', 'ccc']);
  });
});
