...
function findUser(userId) {
  // TODO: 从 usersDb 查找用户，用 Maybe 包装（不存在则为 Nothing）
}

// 1b: verifyActive(user) — 校验用户是否激活
function verifyActive(user) {
  // TODO: 校验用户是否激活 —— 非活跃返回 Left('User not active')，活跃返回 Right(user)
}

// 1c: verifyStock(productId, quantity) — 校验库存
function verifyStock(productId, quantity) {
  // TODO: 校验库存 —— 商品不存在返回 Left('Product not found')，库存不足返回 Left('Insufficient stock')，足够返回 Right(product)
}

// 1d: calculateTotal(product, quantity) — 计算总额
function calculateTotal(product, quantity) {
  // TODO: 计算订单总额 product.price * quantity，返回 Right(total)
}
...
function placeOrder(userId, productId, quantity) {
  // TODO: 用 Maybe + Either 串联全链路：findUser → verifyActive → verifyStock → calculateTotal
}
